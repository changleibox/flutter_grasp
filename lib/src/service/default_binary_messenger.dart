...
class _DefaultBinaryMessenger extends BinaryMessenger {
  /// Creates a [_DefaultBinaryMessenger] instance.
  ///
  /// The [delegate] instance must not be null.
  _DefaultBinaryMessenger._(this.delegate);
  ...
  /// `channel` is not set.
  bool checkMessageHandler(String channel, MessageHandler? handler) => _inboundHandlers[channel] == handler;
...
extension MethodChannelExtension on MethodChannel {
  /// Shim for `SupportMethodChannelExtension.setMockMethodCallHandler`.
  void setMockMethodCallHandler(Future<dynamic>? Function(MethodCall call)? handler) {
    if (binaryMessenger is! _DefaultBinaryMessenger) {
      return;
    }
    (binaryMessenger as _DefaultBinaryMessenger).setMockMethodCallHandler(this, handler);
  }

  /// Shim for `SupportMethodChannelExtension.checkMockMessageHandler`.
  bool checkMockMethodCallHandler(Object? handler) {
    if (binaryMessenger is! _DefaultBinaryMessenger) {
      return false;
    }
    return (binaryMessenger as _DefaultBinaryMessenger).checkMockMessageHandler(name, handler);
  }
}
