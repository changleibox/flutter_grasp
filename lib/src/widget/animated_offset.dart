...
import 'package:flutter_grasp/src/rendering/animated_offset.dart';
import 'package:flutter_grasp/src/rendering/animated_shifted_box_boundary.dart';

/// Created by box on 2020/8/22.
///
/// 在位置改变的时候，加个动画，请在这个布局的根布局上加上[AnimatedShiftedBoxParent]以获取到他的准确位置
...
class _AnimatedOffsetState extends State<AnimatedOffset> with SingleTickerProviderStateMixin {
  @override
  Widget build(BuildContext context) {
    return _AnimatedOffset(
...
class _AnimatedOffset extends SingleChildRenderObjectWidget {
  /// Creates a widget that animates its size to match that of its child.
  ///
  /// The [curve] and [duration] arguments must not be null.
  const _AnimatedOffset({
    Key? key,
    Widget? child,
    this.alignment = Alignment.center,
    this.curve = Curves.linear,
  ...
  @override
  RenderAnimatedOffset createRenderObject(BuildContext context) {
    _debugParent(context);
    return RenderAnimatedOffset(
      alignment: alignment,
      duration: duration,
  ...
  @override
  void updateRenderObject(BuildContext context, RenderAnimatedOffset renderObject) {
    _debugParent(context);
    renderObject
      ..alignment = alignment
      ..duration = duration
